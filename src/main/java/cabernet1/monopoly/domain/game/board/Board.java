...
public class Board {
    ...
    private void initiateTiles() {
        ...
        for (int[] circle : circles) {
            boardTiles.get(circle[0]).setNextTile(1, boardTiles.get(circle[0] + 1));
            boardTiles.get(circle[0]).setNextTile(-1, boardTiles.get(circle[1]));
            boardTiles.get(circle[1]).setNextTile(1, boardTiles.get(circle[0]));
            boardTiles.get(circle[1]).setNextTile(-1, boardTiles.get(circle[1] - 1));
            for (int j = circle[0] + 1; j <= circle[1]-1; ++j) {
                boardTiles.get(j).setNextTile(1, boardTiles.get(j + 1));
                boardTiles.get(j).setNextTile(-1, boardTiles.get(j - 1));
            }
        }
    ...
    public Tile getNextTile(Tile curTile, int direction, int numberOfSteps, boolean takeRailRoads) {
    ...
    public ArrayList<ArrayList<Integer>> getPath(Tile from,Tile to,int direction,boolean takeRailRoads){
    ...
    private Tile moveStep(Tile curTile,int direction,boolean takeRailRoads){
    ...
    public Tile nextUnownedProperty(Tile curTile, int direction, boolean takeRailRoads) {
    ...
    public Tile nextRentableProperty(Tile curTile, IPlayer player, int direction, boolean takeRailRoads) {
        Tile startTile=curTile;
        do{
    ...
    public Tile nextNearestCommunityChestOrChanceTile(Tile curTile, int direction, boolean takeRailRoads) {
        Tile startTile=curTile;
        do{
